...
const handler = async (event) => {
    ...
    try {
        if (type === 'header_one'){
            ...
            if (existingEmailUser) {
                return {
                    statusCode: 400,
                    headers: headers,
                    body: JSON.stringify({ status: "ERROR", message: "Email already exists", data: null }),
                };
            }
        ...
        return {
            statusCode: 500,
            headers: headers,
            body: JSON.stringify({ status: "ERROR", message: "An error occurred while updating contact", data: error }),
        };
    }
};
