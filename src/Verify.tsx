...
const Verify: FC = () => {
    ...
    const verify = async (token: string) => {
        try {
            const response = await fetch(`https://f60z27ge89.execute-api.us-east-1.amazonaws.com/${STAGE}/verify`, {
                method: 'POST',
                headers: {
    ...
    return (
        ...
        <div className="fixed left-0 top-0 z-30 h-screen w-screen">
            ...
            {verified === true ? (<StatusBar message={'Email verified!'} color={'bg-green-500'} />) : verified === false ? (<StatusBar message={'Error verifying email...'} color={'bg-red-500'} />) : null }
        </>
    )
}
