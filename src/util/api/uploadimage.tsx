...
const handleFileUpload = async (
  ...
  setPageData:
    | Dispatch<SetStateAction<AnyPageData>>
    | Dispatch<SetStateAction<UserData>>,
  column: string,
  setShowError: Dispatch<SetStateAction<{ visible: boolean; message: string }>>,
  setCacheBuster: Dispatch<SetStateAction<string>>,
  table: string,
  link: string,
  location: ((prev: AnyPageData) => any) | null = null,
  id: string | null = null
) => {
  ...
  reader.onload = async () => {
    ...
    try {
      const response = await fetch(
        `https://f60z27ge89.execute-api.us-east-1.amazonaws.com/${STAGE}/upload-image?table=${table}&link=${link}&image_column=${column}&id=${
          id || ""
        }`,
        {
          method: "POST",
